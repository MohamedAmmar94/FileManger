...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.invitable.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.invitables.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="invitable">{{ trans('cruds.invitable.fields.invitable') }}</label>
                <input class="form-control {{ $errors->has('invitable') ? 'is-invalid' : '' }}" type="number" name="invitable" id="invitable" value="{{ old('invitable', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('invitable_type') ? 'is-invalid' : '' }}" type="text" name="invitable_type" id="invitable_type" value="{{ old('invitable_type', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('invited_by') ? 'is-invalid' : '' }}" name="invited_by_id" id="invited_by_id" required>
                    @foreach($invited_bies as $id => $entry)
                        <option value="{{ $id }}" {{ old('invited_by_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
